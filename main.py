...
def custom():
    ...
    pieces = [p.Rect(size * 3.125, size * 10, size * 3.75, size * 3), p.Rect(size * 8.125, size * 10, size * 3.75, size * 3),  p.Rect(size * 13.125, size * 10, size * 3.75, size * 3)]
    ...
    while True:
        ...
        for button, asset in zip(modes + pieces + [play], ['default_mode', 'uncorrectable_mode', '123minoes', '4minoes', '5minoes']):
...
def update():
    ...
    for x in range(0, size * 10, block):
    ...
    for y in [size * 2, size * 9.5]:
        SCREEN.blit(p.transform.scale(bg, (size * 5, size * 5)), (size * 12.5, y))
        p.draw.lines(SCREEN, (50, 50, 50), True, [(size * 12.5, y), (size * 17.5, y), (size * 17.5, y + 5 * size), (size * 12.5, y + 5 * size)], 5)
...
def game(settings = [0, [1]]):
    global hold, level, points, lines_counter, data, field, figures, keys, next_figure, block, field_size, bg

    figures, multiplier = figures_loader.main(settings[1])
